...
bool Shader::prep(Visual* target)
{
    ...
    for (int i = Painter::getAttribsMaxIndex(); i >= 0; i--)
    {
        glDisableVertexAttribArray(i);
        if (!CHECK("prep"))
        {
            std::cout << "enabledAttribsMaxValue probably incorrectly set, current value = " << Painter::getVaoID() << std::endl;
        }
    }

    //prepare new attributes, and update variable accordingly
    Painter::setAttribsMaxIndex(this->prepAttribs(target));
