...
Camera::Camera(std::string name, cameraType cameraType, glm::vec3* location, glm::vec3* target) : name(name)
{
	//this->viewMatrix = glm::perspective(45.0f, (GLfloat)winWid / (GLfloat)winHei, 0.1f, 1000.0f);

	if (cameraType == cameraType::DEFAULT)
		cameraType = Cameraman::defaultCameraType;

	switch (cameraType)
	{
	case cameraType::UNSET:
		//error message
		LOGALERT("unset cameraType used in cameraConstructor");
		break;
	...
	case cameraType::ISOMETRIC:
		this->projectionMatrix = glm::orthoRH(-1.0f * Cameraman::defaultAspectRatio, 1.0f * Cameraman::defaultAspectRatio, -1.0f, 1.0f, 0.1f, 1000.0f);
		this->projReady = true;
		break;
	}
	registerCamera();
